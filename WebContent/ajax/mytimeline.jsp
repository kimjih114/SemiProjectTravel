					...
					success: function(data){
						if(data!=null){
							...
							var html = "";
								...
								html+="<table class='tbl-boardsns1' id='boardNo"+data.boardSNS.boardNo+"'>"
								html+="<tr>";
...
function pageMore(cnt){
	...
	$.ajax({
		url: "<%=request.getContextPath()%>/gson/sns/myTimeLine.do",
		data: param,
		type: "get",
		dataType: "json",
		success: function(data){
			if(data!=null){
				...
				$.each(data,(i,tl)=>{
					...
					html+="<table class='tbl-boardsns1' id='boardNo"+tl.boardSNS.boardNo+"'>"
					html+="<tr>";
					html+="<td class='timeline-boardcontent-sns'>";
					...
					if(data.length!=0){
						html+="<div id='trMore1'>더보기</div>";
					}
				}
				
			$("#myTimeLine").append(html);
			
			if($('.tbl-boardsns1').length>0){
				if(($('.tbl-boardsns1').length)<cnt*5){
					$("#trMore1").remove();
				}
			}
			
			$("#trMore1").click(function(e){
				cnt = cnt+1;
				pageMore(cnt);
				$(this).remove();
			});
...
<div class="timeline-board-sns" id="myTimeLine">
